...
def run_agent1(
    ...
    # ── Run Steps ──
    _SKIPPED = {"status": "skipped", "message": "Step explicitly skipped via skip_steps"}

    agent1_payload: Dict[str, Any] = {
        "run_started_at"    : datetime.now(timezone.utc).isoformat(),
        "competitor_profile": _SKIPPED if "A" in skip else {},
        "user_conversations": _SKIPPED if "B" in skip else {},
        "internal_signals"  : _SKIPPED if "C" in skip else {},
    }
    ...
    # Collect all leaf status values for a verdict
    leaf_statuses = set()

    for key in ("competitor_profile", "internal_signals"):
        v = agent1_payload.get(key, {})
        if isinstance(v, dict):
            leaf_statuses.add(v.get("status", "unknown"))

    uc = agent1_payload.get("user_conversations", {})
    if isinstance(uc, dict) and uc.get("status") != "skipped":
        for v in uc.values():
            if isinstance(v, dict):
                leaf_statuses.add(v.get("status", "unknown"))
    else:
        leaf_statuses.add(uc.get("status", "unknown") if isinstance(uc, dict) else "unknown")

    all_explicitly_skipped = skip == ["A", "B", "C"] or leaf_statuses == {"skipped"}
    any_success             = "success" in leaf_statuses

    if all_explicitly_skipped:
        final_status = "agent1_skipped"
    elif any_success:
        final_status = "agent1_done"
    else:
        final_status = "agent1_partial"
    ...
    ready = final_status in ("agent1_done", "agent1_partial", "agent1_skipped")
    log.info(f"\n{'='*55}")
    log.info(f"  Agent 1 complete. Status: {final_status}")
    log.info(f"  Project ID: {project_id}")
    log.info(f"  Ready for Agent 2: {'yes' if ready else 'no'}")
    log.info(f"{'='*55}\n")
