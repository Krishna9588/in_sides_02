...
# Updated to the 5 confirmed-working models (all return 403 with wrong token,
...
def _hf_classify(
    ...
    or None if all fail — caller then uses local model or hybrid.
    """
    if not token:
        return None
    ...
    return None   # all API models failed; caller decides next step
...
def download_model(target_dir: str = LOCAL_MODEL_DIR):
    ...
    Call once:  python agent1_internal.py --download-model
    After that: set USE_LOCAL_MODEL = True and never hit the API again.
    """
    try:
    ...
    try:
        ...
        print(f"  ✓ Now set  USE_LOCAL_MODEL = True  at the top of agent1_internal.py")
        print(f"  ✓ The script will use this model offline for all future runs.\n")
        return True
    except Exception as e:
...
DEMO_HF_TOKEN   = "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"
