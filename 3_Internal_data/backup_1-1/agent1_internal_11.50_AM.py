...
# HuggingFace model cascade — tried in order, first success wins.
# Each entry: (url, payload_format)
#   "zero_shot"  -> uses {"inputs": text, "parameters": {"candidate_labels": [...]}}
#   "text_class" -> uses {"inputs": text}  (maps fixed labels from response)
_HF_MODEL_CASCADE = [
    # Model 1: bart-large-mnli via new router (zero-shot, most accurate)
    (
        "https://router.huggingface.co/hf-inference/models/facebook/bart-large-mnli",
        "zero_shot",
    ),
    # Model 2: bart-large-mnli via legacy inference API (same model, old URL)
    (
        "https://api-inference.huggingface.co/models/facebook/bart-large-mnli",
        "zero_shot",
    ),
    # Model 3: cross-encoder/nli-MiniLM2-L6-H768 — lighter, faster, still zero-shot
    (
        "https://router.huggingface.co/hf-inference/models/cross-encoder/nli-MiniLM2-L6-H768",
        "zero_shot",
    ),
]
# kept for backward compat (used only as first entry above now)
...
def _hf_classify_one(
    ...
    """
    Try a single HF endpoint once (with one retry on 503).
    Returns (top_label, confidence) or None on any failure.
    """
    import urllib.request
    import urllib.error

    if payload_format == "zero_shot":
        body = json.dumps({
            "inputs"    : text,
    ...
    for attempt in range(retries + 1):
        try:
            ...
            if e.code == 503 and attempt < retries:
                time.sleep(4)
                continue
            # 401/403/404 — permanent failure for this endpoint, stop retrying
            log.debug(f"    HF {e.code} on {url}")
            return None
        except Exception as exc:
            log.debug(f"    HF error on {url}: {exc}")
            return None
...
def _hf_classify(
    ...
    candidate_labels: List[str],
    token: str,
    retries: int = 1,           # kept for API compat, used internally above
) -> Optional[Tuple[str, float]]:
    """
    Try each model in _HF_MODEL_CASCADE in order.
    Attempt 1 → Model 1 (bart router)
    Attempt 2 → Model 2 (bart legacy)
    Attempt 3 → Model 3 (MiniLM NLI)
    Attempt 4 → None   (caller uses hardcoded rules)

    Returns (top_label, confidence) from the first model that succeeds,
    or None if all three fail — the caller then uses _classify_rule().
    """
    if not token:
        return None

    for idx, (url, fmt) in enumerate(_HF_MODEL_CASCADE, start=1):
        result = _hf_classify_one(url, fmt, text, candidate_labels, token)
        if result is not None:
            if idx > 1:
                log.debug(f"    HF: used fallback model #{idx}")
            return result
        log.debug(f"    HF model #{idx} failed — trying next ...")

    # All 3 models failed → caller falls back to hardcoded rules
    log.warning("  All HF models failed — using rule-based classifier")
    return None
...
class _Processor:
    ...
    def process(self, file_path: Path) -> InternalResult:
        ...
        # Log HF fallback summary once (not per-turn)
        if hf_fails > 0 and self.token:
            log.warning(
                f"  HF fallback: {hf_fails}/{len(raw_turns)} turns used rule-based "
                f"(all 3 HF models failed or token invalid)"
            )
...
DEMO_HF_TOKEN   = "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"                  # uses the constant at top of file
