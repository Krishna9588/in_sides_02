...
def analyze_video(video_url: str) -> dict:
    ...
    analysis = analyzer(f"""Analyze this YouTube video and return a JSON object with these exact keys:
        ...
        - "relevant_keywords": list of keywords related to confusion, comparison, or problems mentioned in the video (max 10)
        - "decision_and_information": what decisions, choices, or key information is being shared through this video (2-3 sentences)
        - "problems": list of specific problems, issues, or challenges discussed in the video (max 5)
        - "insights": list of deeper insights, learnings, or wisdom shared in the video (max 5)
    ...
    return {
        ...
        "full_transcript": " ".join(s.get("text", "") for s in transcript_parts),
    }
